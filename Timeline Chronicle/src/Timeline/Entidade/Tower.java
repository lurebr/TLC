 ...
 * @author Desenv01
 */
public class Tower extends GameObject implements isAttacker, isDrawable,
		isColide, isSelectable, Cloneable {
	...
	public boolean attack(isAttackable alvo) {
		...
		// C:/Users/Lennon/Documents/NetBeansProjects/timeline-chronicles/Timeline
		// Chronicles/src/Ressource/
		SoundFactory.getSoundPlayer().tocarEfeito("/Sound/shot.wav");
	...
	public void draw(Graphics g) {
		super.animacao.draw(g, (int) super.localizacao.getX(),
				(int) super.localizacao.getY());
		// g.drawRect((int) super.localizacao.getX() - range, (int)
		// super.localizacao.getY() -range, range*2, range*2);
		// System.out.println("X: " + super.localizacao.getX() + " Y:" +
		// super.localizacao.getY());
		this.barraDeAcao.draw(g, (int) super.localizacao.getX(),
				(int) super.localizacao.getY(), super.tamanho.getWidth(),
				super.tamanho.getHeight(), atkSpeedDelay);
		if (selected) {
			Graphics2D g2d = (Graphics2D) g;
			g2d.drawOval((int) super.localizacao.getX(),
					(int) super.localizacao.getY(), range, range);
		}
	}

	@Override
	public boolean colide(isColide obj) {
		if (obj.getLocalizacao().getX() >= this.localizacao.getX() - this.range
				&& obj.getLocalizacao().getX() <= +this.localizacao.getX()
						+ this.range
				&& obj.getLocalizacao().getY() >= this.localizacao.getY()
						- this.range
				&& obj.getLocalizacao().getY() <= +this.localizacao.getY()
						+ this.range) {
	...
	public String Select() {
		String selectText;
		selectText = "Atk:" + this.atributo.getDanoMinimo() + "-"
				+ this.atributo.getDanoMaximo();
		selectText += "\nSpeed:" + this.atkSpeed;
		return selectText;
	}
