...
public class Shot extends GameObject implements isDrawable, isAttack, isColide {
	...
	private void mover(double delta) {
		if (acertou == false) {
			if (alvo.getLocalizacao().getX() != super.localizacao.getX()) {
				if (alvo.getLocalizacao().getX() > super.localizacao.getX()) {
					super.localizacao.setX(super.localizacao.getX()
							+ super.atributo.getSpeed());
				} else if (alvo.getLocalizacao().getX() < super.localizacao
						.getX()) {
					super.localizacao.setX(super.localizacao.getX()
							- super.atributo.getSpeed());
				}
			...
			if (alvo.getLocalizacao().getY() != super.localizacao.getY()) {
				if (alvo.getLocalizacao().getY() > super.localizacao.getY()) {
					super.localizacao.setY(super.localizacao.getY()
							+ super.atributo.getSpeed());
				} else if (alvo.getLocalizacao().getY() < super.localizacao
						.getY()) {
					super.localizacao.setY(super.localizacao.getY()
							- super.atributo.getSpeed());
				}
			...
			if (alvo.getLocalizacao().getX() == super.localizacao.getX()
					&& alvo.getLocalizacao().getY() == super.localizacao.getY()) {
				acertou = true;
			}
		}
	}
	...
	public void draw(Graphics g) {
		...
		// super.animacao.draw(g, super.localizacao.getX(),
		// super.localizacao.getY());
		if (!acertou) {

			g.setColor(Color.red);
			g.drawLine((int) super.localizacao.getX(),
					(int) super.localizacao.getY(),
					(int) super.localizacao.getX() + 10,
					(int) super.localizacao.getY());
	...
	public Dano getDamage() {
		Dano dano = new Dano();
		dano.setDanoProjetil(Matematica.getInstance().getRandom(
				super.atributo.getDanoMinimo(), super.atributo.getDanoMaximo()));
		return dano;
	}
