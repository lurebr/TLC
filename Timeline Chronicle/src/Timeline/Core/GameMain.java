...
public class GameMain extends JFrame {
	...
	private static final long serialVersionUID = -2291023755287210903L;
	...
	private void initialize() {
		...
		// setLoading(this.getGraphics());
		SoundFactory.getSoundPlayer().tocar("/Sound/game.mp3");
	...
	private void gameLoop() {
		...
		while (state != EnumEstado.gameover) {
			...
			case jogando:
				...
				draw();
				try {
					Thread.sleep((tempoUltimoFrame - System.nanoTime() + Parametro.TEMPO_IDEAL) / 1000000);
				} catch (Exception e) {
			...
			if (tempoUltimoFrame >= 1000000000) {
				this.setTitle("Tower Defense - " + "FPS: " + fps + " time:"
						+ getTime());
				tempoUltimoFrame = 0;
				time++;
				fps = 0;
	...
	public void addTower(int posx, int posy, Tower t) {
		...
		if (jogador.getGold() >= 10) {
			if (LevelLoader.getInstance().getMap().getBlock(posx, posy)
					.isAllowTower()) {
				...
				mouse.empty();
				// Tower t = new Tower("Ressource/Object/Tower/tower.png",posx,
				// posy);
				...
				JOptionPane.showMessageDialog(null,
						"Não permitido torre neste bloco");
			}
		}
	}

	public void GameStart(Jogador jogador) {
		GameMain.jogador = jogador;
		initialize();
	}
	...
	private void setLoading(Graphics graphics) {
		...
		graphics.drawString("Loading...",
				Math.abs(Parametro.SCREEN_HEIGHT / 2),
				Math.abs(Parametro.SCREEN_WIDTH / 2));
		graphics.setColor(Color.black);
	...
	private void setPause(Graphics graphics) {
		...
		graphics.fillRect(0, 0, Parametro.SCREEN_HEIGHT, Parametro.SCREEN_WIDTH);
		graphics.setColor(Color.white);
		graphics.drawString("Pause..", Math.abs(Parametro.SCREEN_HEIGHT / 2),
				Math.abs(Parametro.SCREEN_WIDTH / 2));
		graphics.setColor(Color.black);
