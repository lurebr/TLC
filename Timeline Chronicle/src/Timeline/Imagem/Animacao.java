...
public class Animacao implements Cloneable {
	...
	public Animacao(BufferedImage sprite) {
		...
		nSpriteY = sprite.getHeight() / Parametro.SPRITE_HEIGHT;
		nSpriteX = sprite.getWidth() / Parametro.SPRITE_WIDTH;

		for (int i = 0; i < nSpriteY; i++) {
			List<BufferedImage> sprites = new ArrayList<BufferedImage>();
			for (int x = 0; x < nSpriteX; x++) {
				BufferedImage b = sprite.getSubimage(
						Parametro.SPRITE_WIDTH * x,
						Parametro.SPRITE_HEIGHT * i, Parametro.SPRITE_WIDTH,
						Parametro.SPRITE_HEIGHT);
				sprites.add(b);
			}
	...
	public void draw(Graphics g, int x, int y) {
		...
		BufferedImage imagem = listaImagem.get(Integer.toString(estilo)).get(
				frameAtual);
