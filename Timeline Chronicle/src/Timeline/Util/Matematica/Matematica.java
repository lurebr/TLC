...
public class Matematica {
	...
	public int calculaDano(Dano dano, Atributo atributo) {
		...
		danoTotal = Math.abs(dano.getDanoProjetil()
				- atributo.getArmadura().getArmadura(EnumElemento.normal));
		return danoTotal;
	}

	public boolean colidir(isColide obj1, isColide obj2) {
		if (obj1.getLocalizacao().getX() >= obj2.getLocalizacao().getX()
				&& obj1.getLocalizacao().getX() <= obj2.getLocalizacao().getX()
						+ obj2.getTamanho().getWidth()
				&& obj1.getLocalizacao().getY() >= obj2.getLocalizacao().getY()
				&& obj1.getLocalizacao().getY() <= obj2.getLocalizacao().getY()
						+ obj2.getTamanho().getHeight()) {
			return true;
		} else {
			return false;
		}
